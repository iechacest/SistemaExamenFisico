...
<div class="layout">
    ...
    @php
        ...
        // Sexo del postulante: 0 hombre, 1 mujer
        $tituloSexo = $postulante->sexo === 0 ? 'MASCULINO' : 'FEMENINO';

        // Edad aproximada a partir de fecha de nacimiento
        $edad = null;
    ...
    <div class="titulo-ficha">
        FICHA INDIVIDUAL PERSONAL {{ $tituloSexo }}
    </div>
    ...
    <div class="card">
        ...
        {{-- IMPORTANTE: aquí luego pondrás la ruta que guarda prueba + evaluación --}}
        <form method="POST" action="{{ route('postulante.evaluacion', $postulante->id_postulante) }}">
            @csrf

            <table class="ficha">
                <tr>
                    <td class="encabezado-label" style="width: 10%;">C.I.</td>
                    <td style="width: 18%;">
                        <input type="text" name="ci" value="{{ $postulante->ci }}" readonly>
                    </td>
                    <td class="encabezado-label" style="width: 12%;"></td>
                    <td style="width: 22%;">
                    </td>
                    <td class="encabezado-label" style="width: 10%;">FECHA</td>
                    <td>
                ...
                <tr>
                    <td class="encabezado-label">NOMBRES Y APELLIDOS (POSTULANTE):</td>
                    <td colspan="5">
                ...
                <tr>
                    ...
                    <td class="encabezado-label">INSTITUTO AL QUE POSTULA</td>
                    <td>
                        {{ $inst }}
                    </td>
                    <td class="encabezado-label" colspan="2" style="text-align:center;">
                        HUELLA DIGITAL DEL EVALUADO (POSTULANTE)
                    </td>
                </tr>
...
<tr>
    ...
    <td>
        ...
        <small id="velocidad_pts_text">Nota: 0 pts</small>
                ...
                {{-- PRUEBA DE RESISTENCIA (2400 MTS) --}}
<tr>
    <td class="nombre-prueba">
...
<tr>
    <td class="nombre-prueba">
        NATACIÓN ESTILO LIBRE<br><small>(sin tiempo)</small>
    </td>
    <td>
        <input type="number"
               id="natacion"
    ...
    <td>
        ...
        <small id="natacion_pts_text">Nota: 0 pts</small>
                ...
                {{-- CAPACIDAD ABDOMINAL (60 seg) --}}
                {{-- CAPACIDAD ABDOMINAL --}}
<tr>
    <td class="nombre-prueba">
        CAPACIDAD ABDOMINAL<br><small>(120 segundos)</small>
    </td>
    <td>
        <input type="number"
               id="cap_abdominal"
...
<tr>
    <td class="nombre-prueba">
        FLEXIONES DE BRAZO EN SUELO<br><small>(sin tiempo)</small>
    </td>
    <td>
        <input type="number"
               id="flexiones"
    ...
    <td>
        ...
        <small id="flexiones_pts_text">Nota: 0 pts</small>
            ...
            {{-- Diagnóstico, nota total y observaciones --}}
            <div class="fila-extra">
    <div>
        <label for="nota_total">Nota total al 100%</label>
        <input type="number" id="nota_total" name="nota_total" min="0" max="100" placeholder="%"
               readonly>
    </div>
    <div>
        <label for="observacion">Observaciones</label>
        <textarea id="observacion" name="observacion" placeholder="Observaciones adicionales..."></textarea>
    </div>
    <div>
        <label for="promedio">Conclusión</label>
        <input type="text" id="promedio" name="promedio_general" placeholder="APROBADO/REPROBADO"
               readonly>
    </div>
</div>
...
function calcularPromedio() {
    ...
    // Revisamos si alguna nota es menor a 51
